...
import { mapToneToEmojis, getToneVisuals } from './emojiMapper.js';
import { convertByTone, convertEmphasizedWords } from './unicodeConverter.js';
import {
  EnhancedMessage,
  AnalyzedSentence,
  EnhancerSettings,
...
export async function enhanceMessage(
  text: string,
  apiKey?: string,
  settings: Partial<EnhancerSettings> = {}
  ...
  for (const sentence of sentences) {
    ...
    if (opts.enableEmojis && opts.emojiPlacement === 'end_of_sentence') {
      const emojis = mapToneToEmojis(
        tone.emotional.primary,
        tone.style.primary,
        opts.emojiIntensity
      );
      if (emojis) {
  ...
  if (opts.enableEmojis && opts.emojiPlacement === 'end_of_message') {
    ...
    const emojis = mapToneToEmojis(
      overallTone.emotional.primary,
      overallTone.style.primary,
      opts.emojiIntensity
    );
    if (emojis) {
 ...
 */
export function enhanceMessageSync(
  text: string,
  settings: Partial<EnhancerSettings> = {}
): EnhancedMessage {
  const opts = { ...DEFAULT_SETTINGS, ...settings };
 ...
 */
function calculateOverallTone(sentences: AnalyzedSentence[]): ToneAnalysis {
  ...
  const intensity: 'low' | 'medium' | 'high' =
    avgIntensity < 1.5 ? 'low' :
    avgIntensity < 2.5 ? 'medium' : 'high';

  return {
    emotional: {
      primary: dominantEmotional[0] as any,
      confidence: dominantEmotional[1] / totalEmotionalWeight
    },
    style: {
      primary: dominantStyle[0] as any,

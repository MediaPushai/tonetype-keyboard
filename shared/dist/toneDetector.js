 ...
 */
export async function detectTone(text, apiKey) {
    if (!text || !text.trim()) {
 ...
 * @returns ToneAnalysis object
 */
export async function detectToneWithClaude(text, apiKey) {
    if (!text || !text.trim()) {
        return getDefaultTone();
    }
 ...
 */
function validateToneAnalysis(data) {
    const validEmotional = ['happy', 'sad', 'angry', 'excited', 'anxious', 'neutral'];
    const validStyle = ['formal', 'casual', 'sarcastic', 'urgent', 'friendly'];
    ...
    const emphasizedWords = Array.isArray(data.emphasizedWords)
        ? data.emphasizedWords.filter((w) => typeof w === 'string')
        : [];
    return {
        emotional: {
            primary: emotional,
 ...
 */
export function detectToneOffline(text) {
...
//# sourceMappingURL=toneDetector.js.map